...
var client_references_default = {
	...
	"6efdf509a785": async () => {
		const m = await import("./page-Bf0v0hQD.mjs");
		return { get "default"() {
			return m["default"];
		} };
	},
	...
	setRequireModule({ load: async (id) => {
		{
			...
			const deps = __vite_rsc_assets_manifest_default.clientReferenceDeps[id] ?? {
				js: [],
				css: []
			};
			preloadDeps(deps);
...
function preloadDeps(deps) {
	...
	for (const href of deps.css) import_react_dom.preinit(href, {
		as: "style",
		precedence: __vite_rsc_assets_manifest_default.cssLinkPrecedence !== false ? "vite-rsc/client-reference" : void 0
	});
}
	...
	return runWithNavigationContext(async () => {
		...
		try {
			...
			function VinextFlightRoot() {
				...
				const wireElements = (0, import_react.use)(flightRoot);
			...
			const bootstrapScriptContent = await Promise.resolve(__vite_rsc_assets_manifest_default.bootstrapScriptContent);
...
var app_ssr_entry_default = { async fetch(request) {
	...
	const result = await (await import("./rsc.mjs")).default(request);
